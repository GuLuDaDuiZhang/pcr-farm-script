...
def task_40_to_1(device_list: list, account_list1: list, account_list2: list, is_complete_daily: bool,
    ...
    thread_run_action(action.underground_city_battle_and_logout_batch, device_list, account_list1, is_complete_daily,
                      buy_energy_round)
    action.boss_go_to_second_farm(device_list[0], FARM_1_LEADER, FARM_2_NAME)  # 踢掉大号加入农场2
    thread_run_action(action.underground_city_battle_and_logout_batch, device_list, account_list2, is_complete_daily,
                      buy_energy_round)
...
def task_mana_and_daily(device_list: list, account_list: list, farm_name: str, farm_leader: list,
    ...
    thread_run_action(action.underground_city_battle_and_logout_batch, device_list, account_list, is_complete_daily,
                      buy_energy_round)
...
def task_create_guild_and_join(device_list: list, account_list: list, guild_name: str):
    ...
    thread_run_action(action.join_guild_and_logout_batch, device_list, account_list, guild_name)
...
def task_adventure_1_1_hard_3_stars(device_list: list, account_list: list, is_buy_energy: bool):
    # 用来通1-1的3星
    thread_run_action(action.adventure_1_1_hard_3_stars_batch, device_list, account_list, is_buy_energy)
...
def task_adventure_1_1_leve_up(device_list: list, account_list: list, buy_energy_round: int):
    ...
    thread_run_action(action.adventure_1_1_leve_up_batch, device_list, account_list, buy_energy_round)


def task_complete_daily(device_list: list, account_list: list, buy_energy_round: int):
    thread_run_action(action.complete_daily_task_batch, device_list, account_list, buy_energy_round)
