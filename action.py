...
def login(dev: Device, account: list, login_parameters: int = LOGIN_PARAMETERS):
    try:
        ...
        dev.log.warning(e)
        logout(dev)
        login(dev, account, login_parameters)


def underground_city_battle_and_logout(dev: Device, account: list, is_complete_daily: bool, buy_energy_round: int):
    try:
        ...
        dev.log.warning(e)
        logout(dev)
        underground_city_battle_and_logout(dev, account, is_complete_daily, buy_energy_round)


def underground_city_battle_and_logout_batch(dev: Device, account_list: list, is_complete_daily: bool,
                                             buy_energy_round: int):
    for account in account_list:
        if underground_city_battle_and_logout(dev, account, is_complete_daily, buy_energy_round) is fail:
            continue
...
def boss_join_guild_and_set_aid(dev: Device, guild_name: str, boss: list = BOSS):
    try:
        ...
        if join_guild(dev, guild_name, error_logout=False) is fail:
            ...
            if dev.ocr_exists(guild_name, is_full_matching=False, is_accurate=True):
                pass
            else:
                exit()
                return fail
...
def join_guild(dev: Device, guild_name: str, error_logout: bool = False):
    try:
...
def complete_daily_task_batch(dev: Device, account_list: list, buy_energy_round: int = 0):
    for account in account_list:
        if complete_daily_task(dev, account, buy_energy_round) is fail:
            continue


def complete_daily_task(dev: Device, account: list, buy_energy_round: int = 0):
    try:
        if login(dev, account) is fail:
            return fail
        complete_daily(dev, buy_energy_round)
        ...
        dev.log.warning(e)
        logout(dev)
        complete_daily_task(dev, account, buy_energy_round)


def adventure_1_1_leve_up_batch(dev: Device, account_list: list, buy_energy_round: int):
    for account in account_list:
        if adventure_1_1_leve_up(dev, account, buy_energy_round) is fail:
            continue


def adventure_1_1_leve_up(dev: Device, account: list, buy_energy_round: int):
    try:
        if login(dev, account) is fail:
            return fail
        ...
        dev.log.warning(e)
        logout(dev)
        adventure_1_1_leve_up(dev, account, buy_energy_round)


def adventure_1_1_hard_3_stars_batch(dev: Device, account_list: list, is_buy_energy: bool):
    for account in account_list:
        if adventure_1_1_hard_3_stars(dev, account, is_buy_energy) is fail:
            continue


def adventure_1_1_hard_3_stars(dev: Device, account: list, is_buy_energy: bool):
    try:
        if login(dev, account) is fail:
            return fail
        if is_buy_energy:
        ...
        dev.log.warning(e)
        logout(dev)
        adventure_1_1_hard_3_stars(dev, account, is_buy_energy)


def join_guild_and_logout(dev: Device, account: list, guild_name: str):
    try:
        if login(dev, account) is fail:
            return fail
        ...
        dev.log.warning(e)
        logout(dev)
        join_guild_and_logout(dev, account, guild_name)


def join_guild_and_logout_batch(dev: Device, account_list: list, guild_name: str):
    for account in account_list:
        if join_guild_and_logout(dev, account, guild_name) is fail:
            continue
...
def create_guild(dev: Device, leader: list, guild_name: str):
    try:
        ...
        dev.log.warning(e)
        logout(dev)
        create_guild(dev, leader, guild_name)
